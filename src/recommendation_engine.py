...
class SpendingAnalyzer:
    def __init__(self, df: pd.DataFrame):
        self.df = df.copy()
        if "date" in self.df.columns and "month" not in self.df.columns:
            self.df["month"] = pd.to_datetime(self.df["date"]).dt.to_period("M").astype(str)
        if "is_expense" not in self.df.columns and "amount" in self.df.columns:
            self.df["is_expense"] = self.df["amount"] < 0
        self.expenses = self.df[self.df["is_expense"]].copy()
